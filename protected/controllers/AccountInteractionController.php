...
class AccountInteractionController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('confirm', 'confirmNotification', 'passRestore', 'changePass', 'restoreNotification', 'changeNotification'),
				'users'=>array('*'),
			),
	...
	public function actionConfirmNotification()
	{
		if(isset(Yii::app()->session) && isset(Yii::app()->session['regModel'])) {
			...
			$this->render('notification', array('model' => $userModel));
		} else {
			$this->redirect(array(
                    'site/index'
                ));
	...
	public function actionRestoreNotification()
	{
		if(isset(Yii::app()->session) && isset(Yii::app()->session['restoreModel'])) {
			...
			$this->render('restore_notification', array('model' => $userModel));
		} else {
			$this->redirect(array(
					'passRestore'
			));
	...
	public function actionConfirm()
	{
		...
		if($email && $key) {
			...
			if($account === null) {
				...
				Users::model()->updateByPk($confirmedUser->id, array('active' => 1));
		...
		if($email && $key) {
			...
			if($account === null) {
				...
				if (isset($_POST['Users'])) {
					...
					$model->updateByPk($account->user_id, array('password' => $newPassword));
					...
					$this->redirect(array($user->type . '/login', 'newPassword' => 1));
				}
				
				$this->render('change_pass', array('model' => $model));
			}
		} else {
			$this->redirect(array('passRestore'));
		}
	}
}
