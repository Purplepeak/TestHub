...
class ConfirmForm extends CFormModel
{
	...
	public function rules()
	{
		return array(
			...
			array('email, key', 'required', 'message' => 'Поле не должно быть пустым'),
	...
	public function confirmation()
	{
		if(!$this->hasErrors())
		{
			$email = $this->confirmClass->find('email=:email', array(':email' => $this->email));
			$key = $this->confirmClass->find('`key`=:key', array(':key' => $this->key));
